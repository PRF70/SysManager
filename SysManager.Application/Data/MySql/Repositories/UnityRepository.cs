    ...
    public class UnityRepository
    {
        ...
        public async Task<DefaultResponse> DeleteByIdAsync(Guid id)
        {
            ...
            using (var cnx = _context.Connection())
            {
                var result = await cnx.ExecuteAsync(_sql);
                if (result > 0)
                    return new DefaultResponse(id.ToString(), "Unidade de medida excluida com sucesso", false);
            }

            return new DefaultResponse(id.ToString(), "Falha ao tentar excluir um unidade de medida", true);
        }
